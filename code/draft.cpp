...
int main () {
    ...
    */
    ...
    std::vector<std::vector<int>> match_value(nentries);
    std::vector<std::vector<Int_t>> PID(100);
    ...
    */
    ...
    std::vector<TH1F*> histograms{hist1, hist2, hist3, hist4};
    ...
    for(Long64_t i=0; i<nentries; i++) {
        ...
        if (W_t_is_hadronic) {
            ...
            if(((q1_flavour == u)||(q2_flavour == u))&&((q1_flavour == -d)||(q2_flavour == -d))){
                histograms[0]->Fill((truth_q_w_t.Pt()/1000), weight);
                counter_ud+=1;
                }
            if(((q1_flavour == u)||(q2_flavour == u))&&((q1_flavour == -s)||(q2_flavour == -s))){
                histograms[1]->Fill((truth_q_w_t.Pt()/1000), weight);
                counter_us+=1;
                }
            if(((q1_flavour == c)||(q2_flavour == c))&&((q1_flavour == -d)||(q2_flavour == -d))){
                histograms[2]->Fill((truth_q_w_t.Pt()/1000), weight);
                counter_cd+=1;
                }
            if(((q1_flavour == c)||(q2_flavour == c))&&((q1_flavour == -s)||(q2_flavour == -s))){
                histograms[3]->Fill((truth_q_w_t.Pt()/1000), weight);
                counter_cs+=1;
                }
    ...
    // Calculate the percentages using floating-point division
    float percentage_ud = (static_cast<float>(counter_ud) / nentries) * 100;
    float percentage_us = (static_cast<float>(counter_us) / nentries) * 100;
    float percentage_cd = (static_cast<float>(counter_cd) / nentries) * 100;
    float percentage_cs = (static_cast<float>(counter_cs) / nentries) * 100;

    std::cout << "The ud branching ratio is = " << percentage_ud << "%" << std::endl;
    std::cout << "The us branching ratio is = " << percentage_us << "%" << std::endl;
    std::cout << "The cd branching ratio is = " << percentage_cd << "%" << std::endl;
    std::cout << "The cs branching ratio is = " << percentage_cs << "%" << std::endl;
    ...
    std::vector<char*> labels {"u #bar{d}", "u #bar{s}", "c #bar{d}", "c #bar{s}"};
