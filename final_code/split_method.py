...
def CreateHistograms() -> dict:
    ...
    return {
        ...
        "sum_jet_topLep_SV_mass_Electron_basic_final_B": ROOT.TH1D("sum_jet_topLep_SV_mass_Electron_basic_final_B", "sum_jet_topLep_SV_mass_Electron_basic_final_B", sv_nbins, sv_xmin, sv_xmax),
        "sum_jet_topLep_SV_mass_Electron_basic_final_B_passWP77": ROOT.TH1D("sum_jet_topLep_SV_mass_Electron_basic_final_B_passWP77", "sum_jet_topLep_SV_mass_Electron_basic_final_B_passWP77", sv_nbins, sv_xmin, sv_xmax),
        "sum_jet_topLep_SV_mass_Electron_basic_final_B_failWP77": ROOT.TH1D("sum_jet_topLep_SV_mass_Electron_basic_final_B_failWP77", "sum_jet_topLep_SV_mass_Electron_basic_final_B_failWP77", sv_nbins, sv_xmin, sv_xmax),
        ...
        "sum_average_SV_mass_Wjets_Muon_basic_final_B": ROOT.TH1D("sum_average_SV_mass_Wjets_Muon_basic_final_B", "sum_average_SV_mass_Wjets_Muon_basic_final_B", sv_nbins, sv_xmin, sv_xmax),        
        "sum_average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77": ROOT.TH1D("sum_average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77", "sum_average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77", sv_nbins, sv_xmin, sv_xmax),
        "sum_average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77": ROOT.TH1D("sum_average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77", "sum_average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77", sv_nbins, sv_xmin, sv_xmax),
        "sum_average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77": ROOT.TH1D("sum_average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77", "sum_average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77", sv_nbins, sv_xmin, sv_xmax),
        # pT for C/L-enhanced
...
def SelectBenhanced(event, reco: pd.DataFrame, index: int, histograms: dict, relative_weight) -> dict:

    bhad_index = reco["bhad_index"]
    blep_index = reco["blep_index"]

    if event["jet_isbtagged_GN2v01_65"][bhad_index]:

        # Fill the histogram with the total SV mass before splitting (we want hist_total=hist_total_check)
        histograms["jet_topLep_SV_mass_Electron_basic_final_B"].Fill(event["jet_SV_mass"][blep_index][0], event["weight_mc"]*relative_weight)
        histograms["jet_topLep_pt_Electron_basic_final_B"].Fill(event["jet_pt"][blep_index][0], event["weight_mc"]*relative_weight)
        
        #Need to split the data into 2 when they pass/fail the blep tag 77% WP
        if event["jet_GN2v01"][blep_index] > 0.828:

            # Fill the histogram with events that are tagged
            histograms["jet_topLep_SV_mass_Electron_basic_final_B_passWP77"].Fill(event["jet_SV_mass"][blep_index][0], event["weight_mc"]*relative_weight)
            histograms["jet_topLep_pt_Electron_basic_final_B_passWP77"].Fill(event["jet_pt"][blep_index][0], event["weight_mc"]*relative_weight)

            # Split according to truth flavour and fill histograms
            if event["jet_truthflav"][blep_index] == 5:
                histograms["jet_topLep_SV_mass_Electron_basic_final_B_passWP77_b"].Fill(event["jet_SV_mass"][blep_index][0], event["weight_mc"]*relative_weight)
                histograms["jet_topLep_pt_Electron_basic_final_B_passWP77_b"].Fill(event["jet_pt"][blep_index][0], event["weight_mc"]*relative_weight)

            elif event["jet_truthflav"][blep_index] == 4:
                histograms["jet_topLep_SV_mass_Electron_basic_final_B_passWP77_c"].Fill(event["jet_SV_mass"][blep_index][0], event["weight_mc"]*relative_weight)
                histograms["jet_topLep_pt_Electron_basic_final_B_passWP77_c"].Fill(event["jet_pt"][blep_index][0], event["weight_mc"]*relative_weight)

            else:
                histograms["jet_topLep_SV_mass_Electron_basic_final_B_passWP77_l"].Fill(event["jet_SV_mass"][blep_index][0], event["weight_mc"]*relative_weight)
                histograms["jet_topLep_pt_Electron_basic_final_B_passWP77_l"].Fill(event["jet_pt"][blep_index][0], event["weight_mc"]*relative_weight)

        else:

            # Fill the histogram with all the events not b-tagged
            histograms["jet_topLep_SV_mass_Electron_basic_final_B_failWP77"].Fill(event["jet_SV_mass"][blep_index][0], event["weight_mc"]*relative_weight)
            histograms["jet_topLep_pt_Electron_basic_final_B_failWP77"].Fill(event["jet_pt"][blep_index][0], event["weight_mc"]*relative_weight)

            # Split events accotding to their truth flavour and fill histograms
            if event["jet_truthflav"][blep_index] == 5:
                histograms["jet_topLep_SV_mass_Electron_basic_final_B_failWP77_b"].Fill(event["jet_SV_mass"][blep_index][0], event["weight_mc"]*relative_weight)
                histograms["jet_topLep_pt_Electron_basic_final_B_failWP77_b"].Fill(event["jet_pt"][blep_index][0], event["weight_mc"]*relative_weight)

            elif event["jet_truthflav"][blep_index] == 4:
                histograms["jet_topLep_SV_mass_Electron_basic_final_B_failWP77_c"].Fill(event["jet_SV_mass"][blep_index][0], event["weight_mc"]*relative_weight)
                histograms["jet_topLep_pt_Electron_basic_final_B_failWP77_c"].Fill(event["jet_pt"][blep_index][0], event["weight_mc"]*relative_weight)

            else:
                histograms["jet_topLep_SV_mass_Electron_basic_final_B_failWP77_l"].Fill(event["jet_SV_mass"][blep_index][0], event["weight_mc"]*relative_weight)
                histograms["jet_topLep_pt_Electron_basic_final_B_failWP77_l"].Fill(event["jet_pt"][blep_index][0], event["weight_mc"]*relative_weight)
            
    return histograms
    


def SelectCLenhanced(event, reco: pd.DataFrame, index: int, histograms: dict, relative_weight) -> dict:
    ...
    if event["jet_GN2v01"][bhad_index]>0.828 and event["jet_GN2v01"][blep_index]>0.828:

        # Fill histograms before splitting
        histograms["average_SV_mass_Wjets_Muon_basic_final_B"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
        histograms["jet_W1_pt_Muon_basic_final_B"].Fill((event["jet_pt"][q1_index][0]), event["weight_mc"]*relative_weight)
        histograms["jet_W2_pt_Muon_basic_final_B"].Fill((event["jet_pt"][q2_index][0]), event["weight_mc"]*relative_weight)
        ...
        # 2 B-TAGS
        if event["jet_GN2v01"][q1_index]>0.828 and event["jet_GN2v01"][q2_index]>0.828:
            # Fill the histogram with 2 b-tags
            histograms["average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)        
            # Split flavour permutations
            if q1==5 and q2==5:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77_bb"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            elif q1==5 and q2==4:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77_bc"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            elif q1==5 and q2 not in [5,4]:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77_bl"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            elif q1==4 and q2==5:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77_cb"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            elif q1==4 and q2==4:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77_cc"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            elif q1==4 and q2 not in [5,4]:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77_cl"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            elif q1 not in [5,4] and q2==5:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77_lb"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            elif q1 not in [5,4] and q2==4:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77_lc"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            else:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77_ll"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)

        # 1 B-TAG
        elif event["jet_GN2v01"][q1_index]>0.828 or event["jet_GN2v01"][q2_index]>0.828:
            # Fill the histogram with 1 b-tag
            histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            # Divide the events by whether the first is b-tagged or the second is
            if event["jet_GN2v01"][q1_index]>0.828:
                # Split flavour permutations
                if q1== 5 and q2==5:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_bxb"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                elif q1==5 and q2==4:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_bxc"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                elif q1==5 and q2 not in [5,4]:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_bxl"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                elif q1==4 and q2==5:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_cxb"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                elif q1==4 and q2==4:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_cxc"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                elif q1==4 and q2 not in [5,4]:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_cxl"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                elif q1 not in [5,4] and q2==5:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_lxb"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                elif q1 not in [5,4] and q2==4:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_lxc"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                else:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_lxl"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                
            elif event["jet_GN2v01"][q2_index]>0.828:
                # Split flavour permutations
                if q1==5 and q2==5:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_bbx"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                elif q1==5 and q2==4:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_bcx"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                elif q1==5 and q2 not in [5,4]:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_blx"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                elif q1==4 and q2==5:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_cbx"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                elif q1==4 and q2==4:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_ccx"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                elif q1==4 and q2 not in [5,4]:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_clx"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                elif q1 not in [5,4] and q2==5:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_lbx"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                elif q1 not in [5,4] and q2==4:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_lcx"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
                else:
                    histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_llx"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)

        # 0 B-TAGS
        else:
            # Fill the histogram with 0 b-tags
            histograms["average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            # Split flavour permutations
            if q1==5 and q2==5:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77_bb"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            elif q1==5 and q2==4:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77_bc"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            elif q1==5 and q2 not in [5,4]:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77_bl"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            elif q1==4 and q2==5:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77_cb"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            elif q1==4 and q2==4:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77_cc"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            elif q1==4 and q2 not in [5,4] :
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77_cl"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            elif q1 not in [5,4] and q2==5:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77_lb"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            elif q1 not in [5,4] and q2==4:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77_lc"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
            else:
                histograms["average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77_ll"].Fill((event["jet_SV_mass"][q1_index][0] + event["jet_SV_mass"][q2_index][0])/2, event["weight_mc"]*relative_weight)
        ...
        if event["jet_GN2v01"][q1_index] > 0.828:
            ...
            histograms["jet_W1_pt_Muon_basic_final_B_passWP77"].Fill(event["jet_pt"][q1_index][0], event["weight_mc"]*relative_weight)
            # Split by flavour
            if event["jet_truthflav"][q1_index] == 5:
                histograms["jet_W1_pt_Muon_basic_final_B_passWP77_b"].Fill(event["jet_pt"][q1_index][0], event["weight_mc"]*relative_weight)
            elif event["jet_truthflav"][q1_index] == 4:
                histograms["jet_W1_pt_Muon_basic_final_B_passWP77_c"].Fill(event["jet_pt"][q1_index][0], event["weight_mc"]*relative_weight)
            else:
                histograms["jet_W1_pt_Muon_basic_final_B_passWP77_l"].Fill(event["jet_pt"][q1_index][0], event["weight_mc"]*relative_weight)
        else:
            # Fill histogam with non-tagged jets
            histograms["jet_W1_pt_Muon_basic_final_B_failWP77"].Fill(event["jet_pt"][q1_index][0], event["weight_mc"]*relative_weight)
            # Split by flavour
            if event["jet_truthflav"][q1_index] == 5:
                histograms["jet_W1_pt_Muon_basic_final_B_failWP77_b"].Fill(event["jet_pt"][q1_index][0], event["weight_mc"]*relative_weight)
            elif event["jet_truthflav"][q1_index] == 4:
                histograms["jet_W1_pt_Muon_basic_final_B_failWP77_c"].Fill(event["jet_pt"][q1_index][0], event["weight_mc"]*relative_weight)
            else:
                histograms["jet_W1_pt_Muon_basic_final_B_failWP77_l"].Fill(event["jet_pt"][q1_index][0], event["weight_mc"]*relative_weight)

        # Q2
        if event["jet_GN2v01"][q2_index] > 0.828:
            # Fill the histogram with tagged jets
            histograms["jet_W2_pt_Muon_basic_final_B_passWP77"].Fill(event["jet_pt"][q2_index][0], event["weight_mc"]*relative_weight)
            # match statement starts here .
            if event["jet_truthflav"][q2_index] == 5:
                histograms["jet_W2_pt_Muon_basic_final_B_passWP77_b"].Fill(event["jet_pt"][q2_index][0], event["weight_mc"]*relative_weight)
            elif event["jet_truthflav"][q2_index] == 4:
                histograms["jet_W2_pt_Muon_basic_final_B_passWP77_c"].Fill(event["jet_pt"][q2_index][0], event["weight_mc"]*relative_weight)
            else:
                histograms["jet_W2_pt_Muon_basic_final_B_passWP77_l"].Fill(event["jet_pt"][q2_index][0], event["weight_mc"]*relative_weight)
        else:
            # Fill the histogram with non-tagged jets
            histograms["jet_W2_pt_Muon_basic_final_B_failWP77"].Fill(event["jet_pt"][q2_index][0], event["weight_mc"]*relative_weight)
            # match statement starts here .
            if event["jet_truthflav"][q2_index] == 5:
                histograms["jet_W2_pt_Muon_basic_final_B_failWP77_b"].Fill(event["jet_pt"][q2_index][0], event["weight_mc"]*relative_weight)
            elif event["jet_truthflav"][q2_index] == 4:
                histograms["jet_W2_pt_Muon_basic_final_B_failWP77_c"].Fill(event["jet_pt"][q2_index][0], event["weight_mc"]*relative_weight)
            else:
                histograms["jet_W2_pt_Muon_basic_final_B_failWP77_l"].Fill(event["jet_pt"][q2_index][0], event["weight_mc"]*relative_weight)
    return histograms
...
def SumHistograms(histograms: dict) -> dict:
    ...
    histograms["sum_average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77"] += histograms["average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77_bb"] \
                                                                           ...
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77_cl"] \
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_2bTWP77_lc"] \
    ...
    histograms["sum_average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77"] += histograms["average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77_bb"] \
                                                                           ...
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77_cl"] \
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_0bTWP77_lc"] \
    ...
    histograms["sum_average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77"] += histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_bxb"] \
                                                                           ...
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_cxl"] \
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_lxc"] \
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_bxl"] \
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_lxb"] \
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_lxl"] \
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_bbx"] \
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_ccx"] \
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_bcx"] \
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_cbx"] \
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_clx"] \
                                                                           + histograms["average_SV_mass_Wjets_Muon_basic_final_B_1bTWP77_lcx"] \
...
def ClassifyEvents(root: list, reco: list, histograms: dict, settings) -> dict:
    # CREATE LISTS TO HOLD RELEVANT INFO
    b_enhanced = []
    cl_enhanced = []

    luminosity = 29049.3 #pb^-1 for 2022 run
    relative_weight = (luminosity*settings.cross_section)/settings.n_simulated_events
    ...
    for file_index in range(len(root)):
        ...
        for event_index in tqdm(range(nentries), desc='Classifying events', unit='event'):
            ...
            if len(event["el_pt"])==1:
                histograms = SelectBenhanced(event, df.loc[[event_index]], event_index, histograms, relative_weight)

            if len(event["mu_pt"])==1:
                histograms = SelectCLenhanced(event, df.loc[[event_index]], event_index, histograms, relative_weight)
