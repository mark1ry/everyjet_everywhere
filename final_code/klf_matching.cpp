...
TTree * SetTruthTree(char* file_path) {
    ...
    chain->Add("/eos/user/m/moriolpe/mphys_project/root_files/herwig_calib_merged.root");
...
int main () {
    ...
    /* INPUT DATA */
    char* file_path= "/eos/user/m/moriolpe/mphys_project/root_files/herwig_calib_merged.root";
    ...
    TFile* output_file = new TFile("/eos/user/m/moriolpe/mphys_project/root_files/final_files/matched_gn_herwig_4jets_trial.root", "RECREATE");
    ...
    nominal->SetBranchAddress("jet_isbtagged_GN2v01_65", &jet_isbtagged_GN2v01_65);  
    nominal->SetBranchAddress("jet_isbtagged_GN2v01_70", &jet_isbtagged_GN2v01_70);  
    nominal->SetBranchAddress("jet_GN2v01", &jet_GN2v01);
    nominal->SetBranchAddress("jet_GN2v01_pb", &jet_GN2v01_pb);
    nominal->SetBranchAddress("jet_GN2v01_pc", &jet_GN2v01_pc);
    nominal->SetBranchAddress("jet_GN2v01_pu", &jet_GN2v01_pu);
    nominal->SetBranchAddress("jet_GN2v01_pu", &jet_GN2v01_ptau);
    ...
    output_tree->Branch("jet_isbtagged_GN2v01_65", &jet_isbtagged_GN2v01_65);  
    output_tree->Branch("jet_isbtagged_GN2v01_70", &jet_isbtagged_GN2v01_70);  
    output_tree->Branch("jet_GN2v01", &jet_GN2v01);
    output_tree->Branch("jet_GN2v01_pb", &jet_GN2v01_pb);
    output_tree->Branch("jet_GN2v01_pc", &jet_GN2v01_pc);
    output_tree->Branch("jet_GN2v01_pu", &jet_GN2v01_pu);
    output_tree->Branch("jet_GN2v01_pu", &jet_GN2v01_ptau);
    ...
    for(Long64_t i=0; i<10000; i++) {
		...
		if (klfitter_model_bhad_jetIndex->size()>1) {
			for (int j=0; j<klfitter_model_bhad_jetIndex->size(); j++) {
				int temp = (*klfitter_model_bhad_jetIndex)[j];
				std::cout <<  temp << "   ";
			}
			std::cout << std::endl;
		}
	    ...
	    for (int jet{0}; jet<njets; jet++) {
		    if ((*jet_isbtagged_GN2v01_65)[jet] == true){
			    tagged = true;
			}
	    }
        if (!tagged) continue;
		if (njets!=4) continue;
        //if (!mujets_gn_2022 && !mujets_2022_gn_lowPt && !ejets_2022_gn && !ejets_2022_gn_lowPt) continue;
        selected+=1;
